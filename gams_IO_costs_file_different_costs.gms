...
$gdxin IO_traffic_data_gdx_iteration_%NUMBER%.gdx
...
parameter
c(p,i)
$load c = IO_costs_c_mat
c_hat(p,i)
$load c_hat = IO_costs_c_hat_mat
f(j)
D(j,i)
...
x_inequals(p,i) .. c(p,i)*(sum(p1,x(p1,i))) + c(p,i)*x(p,i) + c_hat(p,i) + sum(j,D(j,i)*v(p,j)) + u(i) =g= 0;
...
File traffic_results /IO_traffic_results_%NUMBER%.csv/;
