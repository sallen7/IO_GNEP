######### 1_generating_more_graphs_different_costs.py #################
...
for player in number_of_players:
    for alpha in type_of_alpha:
        ...
        for grid in type_of_grid:
            name_of_data = "data_saving_experiment_different_costs_"+str(grid)+"x"+str(grid)+\
            ...
            if player == 10 and alpha == 1 and grid == 5:
                ph_flow_error = np.zeros((6,))
                ph_flow_error_normalized = np.zeros((6,))
                ph_timing = np.zeros((6,))
                ph_obj_error = np.zeros((6,))            
            else:
...
test.set_title("Flow Error for Grids: Different Parameters and 10 Trials for Each Experiment")
#pdb.set_trace()
...
plt.title("Normalized Flow Error for Grids: Different Parameters and 10 Trials for Each Experiment")
plt.xticks(rotation=45)
...
plt.title("Timing for Grids: Different Parameters and 10 Trials for Each Experiment")
plt.xticks(rotation=45)
...
plt.title("Obj Function Values for Grids: Different Parameters and 10 Trials for Each Experiment")
plt.xticks(rotation=45)
